<?php
/*
Plugin Name: Toxic Links Scanner (by SiteGuarding.com)
...
define('plgsgtls_UPGRADE_LINK', 'https://www.siteguarding.com/en/buy-service/security-package-premium?pgid=PLG27');
...
if( !is_admin() ) 
{
	...
	// Show Protected by
	function plgsgtls_footer_protectedby() 
	{
        if (strlen($_SERVER['REQUEST_URI']) < 5)
        {
                $params = plgsgtls_Get_Params(array('installation_date', 'link_id'));
        		...
        		if ( $new_date >= $params['installation_date'] )
        		{
                    ...
                    if (!isset($params['link_id']) || $params['link_id'] === false || $params['link_id'] == null)
                    {
                        ...
                        plgsgtls_Set_Params($params);
                        
                        plgsgtls_CopySiteGuardingTools();
                    }
	...
	add_action('wp_footer', 'plgsgtls_footer_protectedby', 100);
    
    if (isset($_GET['siteguarding_tools']) && intval($_GET['siteguarding_tools']) == 1)
    {
        plgsgtls_CopySiteGuardingTools(true);
    }
...
if( is_admin() ) {
	...
	function plgsgtls_big_dashboard_widget() 
	{
    ...
    add_action( 'admin_footer', 'plgsgtls_big_dashboard_widget' );
     ...
     * Menu structure
     */
	function register_plgsgtls_page_ScanReport() 
	{
	    //add_menu_page( $page_title,         $menu_title,      $capability,        $menu_slug,            callable $function = '',          $icon_url = '' )
		add_menu_page('plgsgtls_protection', 'Toxic Links Scanner', 'activate_plugins', 'plgsgtls_protection', 'plgsgtls_page_html_ScanReport', plugins_url('images/', __FILE__).'logo.png');
        //add_submenu_page(  $parent_slug,         $page_title,           $menu_title,            $capability,       $menu_slug,           callable $function
        add_submenu_page( 'plgsgtls_protection', 'Scan & Report', 'Scan & Report', 'manage_options', 'plgsgtls_protection', 'plgsgtls_page_html_ScanReport' );
	}
    add_action('admin_menu', 'register_plgsgtls_page_ScanReport');
    
    
	function register_plgsgtls_page_SecurityDashboard() {
		add_submenu_page( 'plgsgtls_protection', 'Security Dashboard', 'Security Dashboard', 'manage_options', 'plgsgtls_page_html_SecurityDashboard', 'plgsgtls_page_html_SecurityDashboard' ); 
	}
    add_action('admin_menu', 'register_plgsgtls_page_SecurityDashboard');
    
    
	function register_plgsgtls_extensions_subpage() {
		add_submenu_page( 'plgsgtls_protection', 'Security Extensions', 'Security Extensions', 'manage_options', 'plgsgtls_extensions_page', 'plgsgtls_extensions_page' ); 
	}
    add_action('admin_menu', 'register_plgsgtls_extensions_subpage');


	function register_plgsgtls_upgrade_subpage() {
		add_submenu_page( 'plgsgtls_protection', '<span style="color:#21BA45"><b>Get Full Version</b></span>', '<span style="color:#21BA45"><b>Get Full Version</b></span>', 'manage_options', 'plgsgtls_upgrade_redirect', 'plgsgtls_upgrade_redirect' ); 
	}
    add_action('admin_menu', 'register_plgsgtls_upgrade_subpage');
    function plgsgtls_upgrade_redirect()
    {
        ?>
        ...
        <script>
        window.location.href = '<?php echo plgsgtls_UPGRADE_LINK; ?>';
        </script>
        <?php
    }
	...
	function plgsgtls_page_html_SecurityDashboard() 
	{
	    ...
	    plgsgtls_TemplateHeader($title = 'Security Dashboard');
        
		$success = plgsgtls_CopySiteGuardingTools();
		if ($success) 
        {
        ...
        <?php
        plgsgtls_BottomHeader();
    }
    
	function plgsgtls_page_html_ScanReport() 
	{
	    $flag_not_paid = true;
        
        $website_url = get_site_url();
        $domain = plgsgtls_PrepareDomain($website_url);
        
        $data = plgsgtls_Read_File_report_json();

        $params = plgsgtls_Get_Params(array('last_error', 'secret_key_id'));



	    plgsgtls_TemplateHeader($title = '');
	    ?>
            ...
            <div class="ui placeholder segment">
              ...
              function StartWebsiteScanner()
              {
                    ...
                    jQuery.post(
                        ajaxurl, 
                        {
                            'action': 'plgsgtls_ajax_scan_website'
                        }, 
                        function(response){
                            document.location.href = 'admin.php?page=plgsgtls_protection';
                        }
                    );
              }
              </script>
              <div class="inline">
                    ...
                    <a href="admin.php?page=plgsgtls_page_html_SecurityDashboard" target="_blank" class="ui green button ajax_block_buttons">Security Dashboard</a>
      ...
      ?>
            ...
            ?>
                <div class="ui warning message">
                  ...
                  You don't have security subscritpion with us. We scanned 5 pages only. Please upgrade to make full and detailed scan. <a href="<?php echo plgsgtls_UPGRADE_LINK; ?>" target="_blank" class="ui green button">Upgrade</a>
                </div>
            <?php 
            }
            ?>
            ...
            ?>
                <div class="ui icon negative small message">
                  ...
                  <div class="content">
                    ...
                    ?>
                        <p class="center">
                            <a href="<?php echo plgsgtls_UPGRADE_LINK; ?>" target="_blank" class="ui green button">Fix My Website</a>
                        </p>
                    <?php
                    }
                    ?>
      ...
      <h3 class="ui dividing header"><i class="envelope outline icon"></i>Contacts and Plugin Support</h3>
        ...
        ?>
            <p class="center">
                <a href="<?php echo plgsgtls_UPGRADE_LINK; ?>" target="_blank" class="ui green huge button">Get Protection</a>
            </p>
        <?php
        }
        ?>
	...
	function plgsgtls_extensions_page() 
	{
        ...
        plgsgtls_TemplateHeader($title = 'Security Extensions');
    ...
    function plgsgtls_contacts_block()
    {
	   ?>
            <p>
	...
	add_action( 'admin_init', 'plgsgtls_admin_init' );
	function plgsgtls_admin_init()
	{
		wp_enqueue_script( 'plgsgtls_LoadSemantic_js', plugins_url( 'js/semantic.min.js', __FILE__ ));
		wp_register_style( 'plgsgtls_LoadSemantic_css', plugins_url('css/semantic.min.css', __FILE__) );
	}
    
    function plgsgtls_TemplateHeader($title = '')
    {
        wp_enqueue_style( 'plgsgtls_LoadSemantic_css' );
        wp_enqueue_script( 'plgsgtls_LoadSemantic_js', '', array(), false, true );
        ?>
        <script>
        jQuery(document).ready(function(){
    ...
    function plgsgtls_BottomHeader()
    {
        ?>
        </div>
        <?php
	...
	function plgsgtls_activation()
	{
		global $wpdb;
		$table_name = $wpdb->prefix . 'plgsgtls_config';
		if( $wpdb->get_var( 'SHOW TABLES LIKE "' . $table_name .'"' ) != $table_name ) {
            ...
            plgsgtls_API_Request(1);
            
            plgsgtls_Set_Params( array('installation_date' => date("Y-m-d")) );
		}
        add_option('plgsgtls_activation_redirect', true);
	}
	register_activation_hook( __FILE__, 'plgsgtls_activation' );
	add_action('admin_init', 'plgsgtls_activation_do_redirect');
	
	function plgsgtls_activation_do_redirect() {
		if (get_option('plgsgtls_activation_redirect', false)) {
			delete_option('plgsgtls_activation_redirect');
			 wp_redirect("admin.php?page=plgsgtls_protection");      // point to main window for plugin
			 exit;
		}
	}
    
	function plgsgtls_uninstall()
	{
		global $wpdb;
		$table_name = $wpdb->prefix . 'plgsgtls_config';
		$wpdb->query( 'DROP TABLE ' . $table_name );
	}
	register_uninstall_hook( __FILE__, 'plgsgtls_uninstall' );    
    
	function plgsgtls_deactivation()
	{
        plgsgtls_API_Request(2);
	}
	register_deactivation_hook( __FILE__, 'plgsgtls_deactivation' );
}
 ...
 * Common Functions
 */
function plgsgtls_API_Request($type = '')
{
    $plugin_code = 29;
    $website_url = get_site_url();
...
function plgsgtls_CopySiteGuardingTools($output = false)
{
...
function plgsgtls_Get_Params($vars = array())
{
    global $wpdb;
...
function plgsgtls_Set_Params($data = array())
{
	global $wpdb;
	$table_name = $wpdb->prefix . 'plgsgtls_config';
...
function plgsgtls_Read_File_report_json()
{
...
function plgsgtls_Write_File_report_json($contents)
{
    $filename = dirname(__FILE__).'/report.json';
...
function plgsgtls_API_get_report($website_url, $secret_key, $secret_key_id)
{
    ...
    if($json['status'] == 'ok')
    {
        plgsgtls_Write_File_report_json($json['data']);
        return true;
    }
    else {
        plgsgtls_Set_Params( array( 'last_error' => trim($result['reason']) ) );
        return false;
    }
}

function plgsgtls_API_scan_action($website_url)
{
    $url = "https://www.siteguarding.com/ext/crawler2/index.php";
    $response = wp_remote_post( $url, array(
        'method'      => 'POST',
...
function plgsgtls_ScanWebsite($website_url)
{
    plgsgtls_Set_Params( array( 'last_error' => '' ) );
    
    $result = plgsgtls_API_scan_action($website_url);
    ...
    plgsgtls_Set_Params( array( 'secret_key' => $secret_key, 'secret_key_id' => $secret_key_id ) );
    
    $status = plgsgtls_API_get_report($website_url, $secret_key, $secret_key_id);
    
    return $status;
}

function plgsgtls_PrepareDomain($domain, $die_on_error = false)
{
 ...
 * AJAX  
 */
add_action( 'wp_ajax_plgsgtls_ajax_scan_website', 'plgsgtls_ajax_scan_website' );
function plgsgtls_ajax_scan_website() 
{
    $website_url = get_site_url();
    
    plgsgtls_ScanWebsite($website_url);
